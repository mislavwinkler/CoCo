...
class AnswerCheckerRepositoryImpl @Inject constructor(
    ...
    override fun checkAnswers(): Boolean {
        ...
        selectedAnswers.forEach { studentAnswers ->
            if (!compareQuestionAndAnswers(studentAnswers.question, studentAnswers.answers)) {
                incorrectAnswers += (studentAnswers.question.answers - studentAnswers.answers.toSet()) + (studentAnswers.answers - studentAnswers.question.answers.toSet())
            }
        }
    ...
    override fun addIncorrectAnswerInfo(answers: List<Answer>) =
        answers.map { answer ->
